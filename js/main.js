...
import DamageNumberManager from "./systems/DamageNumberManager.js";
...
class Game {
  ...
  async createNewGame(playerName) {
    ...
    // 初始化伤害数字管理器
    this.damageNumbers = new DamageNumberManager(this.renderer.scene, this.renderer.camera);
  ...
  handleCombatResult(result) {
    ...
    // 显示3D伤害数字
    if (this.settings.showDamageNumbers && this.damageNumbers) {
      if (result.type === "damage") {
        if (result.source === "player" && result.target && result.target.mesh) {
          this.damageNumbers.showDamage(result.damage, result.target.mesh.position, false);
        } else if (result.source === "monster" && this.player.mesh) {
          this.damageNumbers.showDamage(result.damage, this.player.mesh.position, true);
        }
      } else if (result.type === "heal" && this.player.mesh) {
        // 显示治疗数字（即使没有实际治疗量也显示，表示技能生效）
        const displayHeal =
          result.healAmount > 0 ? result.healAmount : Math.floor(this.player.maxHp * (result.healPercent || 0.1));
        // 创建位置副本，避免引用问题
        const pos = this.player.mesh.position.clone();
        console.log("[Main] 显示治疗数字:", displayHeal, "位置:", pos, "result:", result);
        this.damageNumbers.showHeal(displayHeal, pos);
      } else if (result.type === "dodge" && this.player.mesh) {
        // 显示闪避文字
        this.damageNumbers.showDodge(this.player.mesh.position);
      }
    }
    ...
    if (result.killed) {
      ...
      // 掉落物品
      if (result.drops && this.damageNumbers) {
        const dropPosition = result.target?.mesh?.position?.clone() || new THREE.Vector3(0, 0, 0);
        result.drops.forEach((drop, index) => {
          ...
          if (item && this.player.addItem(item, drop.count)) {
            ...
            // 创建3D掉落动画
            setTimeout(() => {
              this.damageNumbers.createDropItem(item, dropPosition);
            }, index * 100);
          }
        });
      }
