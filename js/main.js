...
import GroundClickManager from "./systems/GroundClickManager.js";
...
class Game {
  ...
  async createNewGame(playerName) {
    ...
    // 初始化地面点击管理器
    this.groundClick = new GroundClickManager(this);
  ...
  update(deltaTime) {
    // 更新地面点击管理器
    if (this.groundClick) {
      this.groundClick.update(deltaTime);
    }

    // 获取移动输入（优先使用地面点击移动）
    let direction = { x: 0, z: 0 };
    if (this.groundClick) {
      const clickDirection = this.groundClick.getMovementDirection();
      if (clickDirection) {
        // 使用地面点击移动方向
