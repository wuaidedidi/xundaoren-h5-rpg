...
import { getItem, useItem, getBlacksmithShopItems, calculateRepairCost } from "./data/items.js";
...
class Game {
      ...
      if (e.key === "F8") {
        ...
        const playerData = {
          ...
          inventory: [
            ...
            { itemId: "basicSword", count: 1 },
            { itemId: "basicRobe", count: 1 },
            { itemId: "woodenRing", count: 1 },
          ],
          equipment: {
            weapon: null,
            armor: null,
  ...
  handleDialogOption(option) {
    ...
    switch (action) {
      ...
      case "openShopBuy":
        this.ui.hideDialog();
        this.ui.showShopPanel('buy', this.player);
        break;

      case "openShopSell":
        this.ui.hideDialog();
        this.ui.showShopPanel('sell', this.player);
        break;

      case "openShopRepair":
        this.ui.hideDialog();
        this.ui.showShopPanel('repair', this.player);
        break;
  ...
  handleItemClick(item, index) {
    ...
    if (item.type === "consumable") {
      ...
      // 装备物品
      const result = this.player.equipItem(item.id, index);
      if (result.success) {
        this.ui.showToast(result.message, "success");
        this.ui.updateInventoryPanel(this.player);
        this.ui.updateCharacterPanel(this.player);
  ...
  handleEquipmentSlotClick(slot) {
    ...
    if (!equipped || !equipped.itemId) {
      this.ui.showToast("该位置没有装备", "info");
      return;
    }
  ...
  handleBuyItem(itemId) {
    ...
    // 扣除金币
    this.player.spendGold(item.price);
    this.ui.showToast(`购买成功：${item.name}`, "success");
    
    // 更新UI
    this.ui.updateShopGold(this.player);
    this.ui.updateInventoryPanel(this.player);
    this.ui.updatePlayerHUD(this.player);
  }
  ...
  handleSellItem(itemId, index) {
    ...
    // 从背包移除
    const removed = this.player.removeItemFromInventory(index);
    if (!removed) {
    ...
    // 获得金币
    this.player.gainGold(sellPrice);
    this.ui.showToast(`出售成功：${item.name} (+${sellPrice}金币)`, "success");
   ...
   * 处理修理装备
   */
  handleRepairItem(slot) {
    const equipped = this.player.equipment[slot];
    if (!equipped || !equipped.itemId) {
      this.ui.showToast("该位置没有装备", "warning");
      return;
    }
    ...
    // 扣除金币
    this.player.spendGold(repairCost);
    this.ui.showToast(`修理成功：${item.name}`, "success");
