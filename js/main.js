...
class Game {
  ...
  async createNewGame(playerName) {
    ...
    // 初始化小地图
    this.minimap = new MiniMap(this.renderer.renderer, this.renderer.scene);
    this.minimap.init();

    // 初始化任务光柱系统
    this.questBeacon = new QuestBeacon(this.renderer.scene);
    this.questBeacon.createBeacon("tutorial", new THREE.Vector3(0, 0, 0), "main");
  ...
  update(deltaTime) {
    ...
    // 更新任务光柱
    if (this.questBeacon) {
      this.questBeacon.update(deltaTime);
    }
  ...
  render() {
    ...
    if (this.minimap && this.player && this.world) {
      const questBeacons = this.questBeacon ? Array.from(this.questBeacon.beacons.values()) : [];
      this.minimap.update(this.player.mesh.position, this.world.npcs, this.world.monsters, questBeacons);
    }
  }
