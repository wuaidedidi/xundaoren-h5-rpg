...
import { apiInterceptor } from "./core/ApiInterceptor.js";
...
class Game {
  ...
  async init() {
    ...
    try {
      // 初始化UI（必须首先初始化，因为其他步骤需要显示进度）
      this.ui = new UIManager(this);

      // 初始化资源加载器
      this.resourceLoader.init();
      this.ui.setLoadingProgress(5, "正在初始化资源管理器...");

      // 初始化网络管理器
      this.networkManager.init();
      this.gameStore.actions.setNetworkStatus(navigator.onLine);
   ...
   */
  startGame() {
    this.gameStore.actions.setRunning(true);
   ...
   */
  bindEventBusEvents() {
    this.eventBus.on("player:hpChange", () => {
      this.ui.updatePlayerHUD(this.player);
    });
    this.eventBus.on("player:expChange", () => {
      this.ui.updatePlayerHUD(this.player);
    });
    this.eventBus.on("player:levelUp", (data) => {
      this.ui.showLevelUp(data.level);
    });
    this.eventBus.on("player:realmUp", (data) => {
      this.ui.showRealmUp(data.realm);
    });
    this.eventBus.on("player:goldChange", (data) => {
      this.ui.showGoldGain(data.amount);
      this.ui.updatePlayerHUD(this.player);
    });
    this.eventBus.on("network:online", () => {
      this.gameStore.actions.setNetworkStatus(true);
      this.ui.showToast("网络已连接", "success");
    });
    this.eventBus.on("network:offline", () => {
      this.gameStore.actions.setNetworkStatus(false);
      this.ui.showToast("网络已断开，操作将暂存本地", "warning");
    });
    this.eventBus.on("game:message", (data) => {
      this.ui.showToast(data.message, data.type || "info");
    });
  }
    ...
    switch (action) {
      ...
      case "learnDragonGrip":
        if (this.player.learnedSkills.includes("dragonGrip")) {
          ...
          this.player.learnSkill("dragonGrip");
  ...
  selectClass(classId) {
    ...
    if (hadClass) {
      ...
      oldSkills.forEach((id) => {
        this.player.unlearnSkill(id);
      });
  ...
  handleItemClick(item, index) {
    ...
    if (item.type === "consumable") {
      ...
      if (result.success) {
        ...
        this.player.removeItem(item.itemId, 1);
        this.ui.updateInventoryPanel(this.player);
        this.ui.updatePlayerHUD(this.player);
      } else {
  ...
  toggleGameMenu() {
    ...
    if (gameMenu) {
      ...
      this.gameStore.actions.setPaused(isHidden);
    }
  }
}
