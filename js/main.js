  ...
  bindInputEvents() {
    ...
    this.input.on("click", (mouse, event) => {
      ...
      if (intersects.length > 0) {
        ...
        const groundIntersects = this.renderer.raycast({ x: mouse.x, y: mouse.y }, this.world.getGroundObjects());

        if (groundIntersects.length > 0) {
          const hitPoint = groundIntersects[0].point;
          this.effects.createGroundMarker({ x: hitPoint.x, z: hitPoint.z });
          this.player.moveToTarget(hitPoint.x, hitPoint.z);
        }
      }
    });
    ...
    this.input.on("mousemove", (mouse, event) => {
      ...
      if (intersects.length > 0) {
        ...
        if (entity && (hit.userData.type === "monster" || hit.userData.type === "npc")) {
          if (this.hoveredObject !== entity) {
            if (this.hoveredObject) {
              this.effects.removeOutlineEffect(this.hoveredObject.mesh);
            }
            this.hoveredObject = entity;
            const color = hit.userData.type === "monster" ? 0xff0000 : 0x00ff00;
            this.effects.addOutlineEffect(entity.mesh, color);
          }
        } else {
          if (this.hoveredObject) {
            this.effects.removeOutlineEffect(this.hoveredObject.mesh);
