...
import QuestBeaconSystem from "./systems/QuestBeaconSystem.js";
...
class Game {
  ...
  async createNewGame(playerName) {
    ...
    // 初始化任务光柱系统
    this.questBeaconSystem = new QuestBeaconSystem(this);
    this.questBeaconSystem.init();

    // 创建一个测试任务光柱（在新手村中央）
    this.questBeaconSystem.setQuestTarget(new THREE.Vector3(0, 0, 0), "main");
  ...
  update(deltaTime) {
    ...
    // 更新任务光柱
    if (this.questBeaconSystem) {
      this.questBeaconSystem.update(deltaTime);
    }
