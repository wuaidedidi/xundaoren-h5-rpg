...
class Game {
  ...
  startGame() {
    ...
    // 创建测试任务光柱
    if (this.questBeacon) {
      this.questBeacon.createBeacon('test-main', { x: 0, z: -5 }, 'main');
      this.questBeacon.createBeacon('test-side', { x: -15, z: 5 }, 'side');
      this.questBeacon.createBeacon('test-daily', { x: 15, z: 5 }, 'daily');
    }
  ...
  update(deltaTime) {
    ...
    // 更新小地图
    if (this.minimap) {
      this.minimap.update(deltaTime, this.player, this.world.npcs, this.world.monsters);
    }
  ...
  render() {
    ...
    // 渲染小地图
    if (this.minimap) {
      this.minimap.render(this.renderer.renderer);
    }
  }
