...
class Game {
  ...
  update(deltaTime) {
    ...
    // 显示持续治疗数字
    if (this.player.lastHealAmount && this.player.lastHealAmount > 0 && this.effects) {
      const position = this.player.mesh.position.clone();
      this.effects.playDamageNumber(position, this.player.lastHealAmount, false, "heal");
      this.player.lastHealAmount = 0;
    }
  ...
  handleCombatResult(result) {
    ...
    // 显示3D伤害数字
    if (result.type === "damage" && this.settings.showDamageNumbers && this.effects) {
      let position;

      if (result.source === "player" && result.target) {
        position = result.target.mesh ? result.target.mesh.position.clone() : result.target.position.clone();
      } else if (result.source === "monster") {
        position = this.player.mesh.position.clone();
      }

      if (position) {
        this.effects.playDamageNumber(position, result.damage, result.isCritical, "damage");
      }
    }

    if (result.type === "heal" && this.effects) {
      const position = this.player.mesh.position.clone();
      this.effects.playDamageNumber(position, result.heal || 0, false, "heal");
    }

    if (result.type === "dodge" && this.effects) {
      const position = this.player.mesh.position.clone();
      this.effects.playDamageNumber(position, 0, false, "dodge");
    }

    if (result.type === "movement" && result.message && result.message.includes("闪避") && this.effects) {
      const position = this.player.mesh.position.clone();
      this.effects.playDamageNumber(position, 0, false, "dodge");
    }
    ...
    if (result.killed) {
      ...
      // 掉落物品
      if (result.drops) {
        const dropPos = result.target.mesh ? result.target.mesh.position.clone() : result.target.position.clone();
        result.drops.forEach((drop) => {
          const item = getItem(drop.itemId);
          if (item && this.player.addItem(item, drop.count)) {
            if (this.effects) {
              this.effects.playDropItem(dropPos, `${item.name} x${drop.count}`);
            }
