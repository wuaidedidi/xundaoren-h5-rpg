...
class Game {
  ...
  update(deltaTime) {
    ...
    // 更新玩家动画状态（如果动画被锁定则不切换）
    if (this.player.animationLockTime <= 0) {
      if (this.player.isMoving) {
        this.player.setAnimationState("move");
      } else if (!this.player.inCombat) {
    ...
    if (result.success) {
      ...
      if (skill) {
        if (skill.type === "physical") {
          this.player.setAnimationState("attack", 0.5); // 攻击动画锁定0.5秒
        } else if (
          skill.type === "magic" ||
          skill.type === "buff" ||
          skill.type === "heal" ||
          ...
          this.player.setAnimationState("cast", 0.8); // 施法动画锁定0.8秒
        }
      }
  ...
  handleCombatResult(result) {
    ...
    // 触发攻击动画（普攻和技能，带锁定时间）
    if (result.source === "player" && result.skill) {
      if (result.skill.type === "physical") {
        this.player.setAnimationState("attack", 0.5);
      } else if (
        result.skill.type === "magic" ||
        result.skill.type === "buff" ||
        result.skill.type === "heal" ||
        result.skill.type === "control"
      ) {
        this.player.setAnimationState("cast", 0.8);
      }
    }
