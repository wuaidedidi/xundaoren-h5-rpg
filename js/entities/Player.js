...
export default class Player {
  ...
  updateBuffs(deltaTime) {
    this.buffs = this.buffs.filter((buff) => {
      ...
      if (buff.type === "hot" && buff.healPerSecond) {
        const healAmount = buff.healPerSecond * deltaTime;
        buff._healAccum = (buff._healAccum || 0) + healAmount;
        const actualHeal = Math.floor(buff._healAccum);
        if (actualHeal > 0) {
          this.hp = Math.min(this.maxHp, this.hp + actualHeal);
          this.lastHealAmount = (this.lastHealAmount || 0) + actualHeal;
          buff._healAccum -= actualHeal;
        }
      }
  ...
  takeDamage(amount) {
    this.dodged = false;
    // 检查闪避概率（10%基础闪避）
    const dodgeChance = 0.1 + (this.agility || 0) * 0.01;
