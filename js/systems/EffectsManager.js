...
export default class EffectsManager {
  ...
  stopRain() {
    ...
    if (this.renderer) {
      this.renderer.setWeather("clear");
    }
  }
  ...
  createRainSystem() {
    ...
    for (let i = 0; i < rainCount; i++) {
      ...
      positions[i3] = (Math.random() - 0.5) * rainRadius * 2;
      positions[i3 + 1] = Math.random() * rainHeight;
      positions[i3 + 2] = (Math.random() - 0.5) * rainRadius * 2;
  ...
  updateRain(deltaTime, cameraPosition) {
    if (!this.rainSystem || !this.isRaining) return;

    const positions = this.rainSystem.geometry.attributes.position.array;
    const velocities = this.rainSystem.velocities;
    const radius = this.rainSystem.rainRadius;
    const height = this.rainSystem.rainHeight;

    const camX = cameraPosition ? cameraPosition.x : 0;
    const camZ = cameraPosition ? cameraPosition.z : 0;
    ...
    for (let i = 0; i < positions.length / 3; i++) {
      ...
      if (positions[i3 + 1] < 0) {
        positions[i3] = camX + (Math.random() - 0.5) * radius * 2;
        positions[i3 + 1] = height + Math.random() * 10;
        positions[i3 + 2] = camZ + (Math.random() - 0.5) * radius * 2;
      }
  ...
  updateLightning(deltaTime) {
    if (!this.isRaining || !this.renderer) return;

    this.lightningTimer += deltaTime;

    if (this.lightningTimer >= this.lightningInterval) {
      this.lightningTimer = 0;
      this.lightningInterval = 5 + Math.random() * 10;
  ...
  createLightningBolt() {
    ...
    while (currentY > endY) {
      ...
      points.push(new THREE.Vector3(currentX, Math.max(currentY, endY), currentZ));
    }
    ...
    glow.position.set(currentX, 1, currentZ);
    this.scene.add(glow);
  ...
  update(deltaTime) {
    try {
      ...
      if (this.isRaining && this.renderer) {
        this.updateRain(deltaTime, this.renderer.camera.position);
