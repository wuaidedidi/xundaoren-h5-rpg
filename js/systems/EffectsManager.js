...
export default class EffectsManager {
  ...
  update(deltaTime) {
    try {
      ...
      this.updateDropAnimations(deltaTime);
    } catch (e) {
  ...
  createFloatingText(position, text, type = "damage") {
    ...
    sprite.position.set(position.x + offsetX, position.y + 2, position.z);

    const baseScale = 2.5;
    const scale = type === "critical" ? 1.5 : 1;
    sprite.scale.set(baseScale * scale, baseScale * scale * 0.5, 1);
  ...
  updateFloatingTexts(deltaTime) {
    ...
    this.floatingTexts = this.floatingTexts.filter((ft) => {
      ...
      if (ft.type === "dodge") {
        ft.sprite.position.x += ft.offsetX * deltaTime * 3;
        ft.sprite.position.y = ft.startY + Math.sin(progress * Math.PI) * 0.3;
      } else {
        ft.sprite.position.y = ft.startY + progress * 2.5;
      }

      ft.sprite.material.opacity = 1 - progress;
   ...
   * 创建掉落物抛物线动画
   */
  createDropAnimation(position, item, onComplete) {
    ...
    this.dropAnimations = this.dropAnimations || [];
  ...
  createDropMesh(item) {
    ...
    switch (item?.type || "misc") {
      case "potion":
        geometry = new THREE.CylinderGeometry(0.15, 0.15, 0.3, 8);
        material = new THREE.MeshLambertMaterial({
          color: item.id.includes("hp") ? 0xff4444 : 0x4444ff,
          emissive: item.id.includes("hp") ? 0x440000 : 0x000044,
        });
        break;
      case "material":
  ...
  updateDropAnimations(deltaTime) {
    ...
    this.dropAnimations = this.dropAnimations.filter((anim) => {
      ...
      const x = anim.startPos.x + (anim.targetPos.x - anim.startPos.x) * t;
      const z = anim.startPos.z + (anim.targetPos.z - anim.startPos.z) * t;

      const height = 4 * anim.peakHeight * t * (1 - t);
      const y = anim.startPos.y + height;

      anim.mesh.position.set(x, y, z);
      anim.mesh.rotation.y += deltaTime * 5;
  ...
  clear() {
    ...
    if (this.dropAnimations) {
      this.dropAnimations.forEach((da) => {
        this.scene.remove(da.mesh);
        da.mesh.geometry.dispose();
        da.mesh.material.dispose();
      });
      this.dropAnimations = [];
    }
  }
