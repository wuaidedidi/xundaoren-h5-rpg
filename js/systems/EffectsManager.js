...
export default class EffectsManager {
  constructor(scene) {
    ...
    this.continuousEffects = new Map();
   ...
   * 创建持续循环粒子效果
   */
  createContinuousEffect(id, options) {
    const effect = {
      id,
      options,
      elapsed: 0,
    ...
    this.continuousEffects.set(id, effect);
    return effect;
  }
   ...
   * 移除持续效果
   */
  removeContinuousEffect(id) {
    const effect = this.continuousEffects.get(id);
    if (effect) {
      ...
      this.continuousEffects.delete(id);
    }
  }
  ...
  updateContinuousEffects(dt) {
    for (const [id, effect] of this.continuousEffects) {
      effect.elapsed += dt;
      effect.spawnTimer += dt;

      const options = effect.options;
      const spawnRate = options.spawnRate || 100;

      if (effect.spawnTimer >= spawnRate) {
        effect.spawnTimer = 0;
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (options.positionVariation) {
            pos.x += (Math.random() - 0.5) * options.positionVariation.x;
            pos.y += (Math.random() - 0.5) * options.positionVariation.y;
            pos.z += (Math.random() - 0.5) * options.positionVariation.z;
          }
          ...
          const velocity = {
            x: (Math.random() - 0.5) * (options.velocitySpread || 1),
            y: (options.velocityY || 0) + Math.random() * (options.velocitySpread || 1),
            z: (Math.random() - 0.5) * (options.velocitySpread || 1),
          };

          let orbitalAngle = 0;
          if (options.orbital) {
            orbitalAngle = Math.random() * Math.PI * 2;
            const radius = options.orbitalRadius || 1.5;
            positions[0] = options.position.x + Math.sin(orbitalAngle) * radius;
            positions[2] = options.position.z + Math.cos(orbitalAngle) * radius;
      ...
      for (let i = effect.particles.length - 1; i >= 0; i--) {
        ...
        const positions = p.points.geometry.attributes.position.array;
        if (p.orbital) {
          const time = effect.elapsed * 0.001;
          const angle = time * (options.orbitalSpeed || 1) + p.orbitalAngle;
          const radius = options.orbitalRadius || 1.5;
          positions[0] = options.position.x + Math.sin(angle) * radius;
          positions[1] = options.position.y + Math.sin(time * 3 + p.orbitalAngle) * 0.3;
          positions[2] = options.position.z + Math.cos(angle) * radius;
        } else {
          positions[0] += p.velocity.x * dt * 0.001;
          positions[1] += p.velocity.y * dt * 0.001;
          positions[2] += p.velocity.z * dt * 0.001;
          p.velocity.y += p.gravity * dt * 0.001;
        }
        p.points.geometry.attributes.position.needsUpdate = true;
