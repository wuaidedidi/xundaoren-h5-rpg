...
export default class EffectsManager {
    ...
    try {
      ...
      for (let i = this.animations.length - 1; i >= 0; i--) {
        ...
        switch (anim.type) {
          ...
          case "shrink":
            const shrinkScale = anim.startScale - (anim.startScale - anim.endScale) * progress;
            anim.object.scale.set(shrinkScale, shrinkScale, 1);
            anim.object.material.opacity = 1 - progress;
            break;
        }
   ...
   * 为物体添加发光描边效果
   */
  addOutlineEffect(object, color = 0x44aaff) {
    if (!object || !object.geometry) return null;

    if (object.userData.outlineMesh) {
      this.removeOutlineEffect(object);
    }

    const outlineGeom = object.geometry.clone();
  ...
  removeOutlineEffect(object) {
    if (!object || !object.userData.outlineMesh) return;

    const outlineMesh = object.userData.outlineMesh;
    ...
    delete object.userData.outlineMesh;
  }
