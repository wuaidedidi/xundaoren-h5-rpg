...
export default class EffectsManager {
  ...
  createClickMarker(position) {
    ...
    // 外圈 - 扩散消失
    const outerRingGeom = new THREE.RingGeometry(0.8, 1.0, 32);
    ...
    outerRing.position.set(position.x, 0.05, position.z);
    ...
    // 中圈 - 收缩
    const middleRingGeom = new THREE.RingGeometry(0.5, 0.7, 32);
    ...
    middleRing.position.set(position.x, 0.05, position.z);
    ...
    // 内圈 - 快速收缩
    const innerRingGeom = new THREE.RingGeometry(0.2, 0.35, 32);
    ...
    innerRing.position.set(position.x, 0.05, position.z);
    ...
    // 中心点 - 闪烁
    const centerGeom = new THREE.CircleGeometry(0.15, 16);
    ...
    center.position.set(position.x, 0.06, position.z);
  ...
  setHoverOutline(entity) {
    ...
    this.hoverOutline = new THREE.Mesh(geometry, outlineMat);
    this.hoverOutline.position.copy(mesh.position);
    this.hoverOutline.rotation.copy(mesh.rotation);
    this.hoverOutline.scale.copy(mesh.scale).multiplyScalar(1.15);
  ...
  update(deltaTime) {
    try {
      ...
      for (let i = this.clickMarkers.length - 1; i >= 0; i--) {
        const marker = this.clickMarkers[i];
        marker.elapsed += dt;

        const progress = Math.min(marker.elapsed / marker.duration, 1);
        ...
        marker.rings.forEach((ringData) => {
          ...
          if (ringData.type === "expand") {
            // 扩散效果
            const scale = 1 + ringData.expandScale * easeProgress;
            ring.scale.setScalar(scale);
            ring.material.opacity = 0.6 * (1 - progress);
          } else {
            // 收缩效果
            const shrinkProgress = easeProgress;
            const innerRadius = ringData.startInner * (1 - shrinkProgress * 0.8);
            const outerRadius = ringData.startOuter * (1 - shrinkProgress * 0.7);

            ring.geometry.dispose();
            ring.geometry = new THREE.RingGeometry(Math.max(0.05, innerRadius), Math.max(0.1, outerRadius), 32);
        ...
        // 更新中心点
        if (marker.center) {
          const flashProgress = Math.sin(progress * Math.PI * 3);
          marker.center.material.opacity = (1 - progress) * (0.5 + flashProgress * 0.5);
          marker.center.scale.setScalar(1 + (1 - progress) * 0.5);
        }
        ...
        if (progress >= 1) {
          ...
          // 清理中心点
          if (marker.center) {
            this.scene.remove(marker.center);
            marker.center.geometry.dispose();
            marker.center.material.dispose();
          }
      ...
      // 更新悬停描边位置
      if (this.hoverOutline && this.hoveredEntity && this.hoveredEntity.mesh) {
        this.hoverOutline.position.copy(this.hoveredEntity.mesh.position);
        this.hoverOutline.rotation.copy(this.hoveredEntity.mesh.rotation);

        // 脉冲效果
        const pulse = 1.12 + Math.sin(Date.now() * 0.005) * 0.03;
        this.hoverOutline.scale.copy(this.hoveredEntity.mesh.scale).multiplyScalar(pulse);
      }
  ...
  clear() {
    ...
    this.clickMarkers.forEach((m) => {
      m.rings.forEach((ringData) => {
        this.scene.remove(ringData.mesh);
        ringData.mesh.geometry.dispose();
        ringData.mesh.material.dispose();
      });
      if (m.center) {
        this.scene.remove(m.center);
        m.center.geometry.dispose();
        m.center.material.dispose();
      }
    });
    this.clickMarkers = [];
