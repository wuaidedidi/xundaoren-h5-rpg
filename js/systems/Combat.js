...
export default class CombatSystem {
  ...
  executeHealSkill(skill) {
    console.log('[Combat] executeHealSkill called:', skill.id);
    if (skill.healPercent) {
      // 立即治疗 + 持续治疗
      const duration = skill.duration || 5;
      const instantHeal = this.player.maxHp * skill.healPercent;
      const actualHeal = this.player.heal(instantHeal);

      console.log('[Combat] Heal calculated:', { instantHeal, actualHeal, maxHp: this.player.maxHp, currentHp: this.player.hp });

      // 添加持续治疗buff
      const healPerSecond = instantHeal / duration;
      ...
      const result = {
        ...
        message: `${skill.name}：回复${Math.floor(actualHeal)}点生命`,
      };
      console.log('[Combat] Heal result:', result);
      return result;
    }

    return { success: true, type: "heal", healAmount: 0 };
  }
