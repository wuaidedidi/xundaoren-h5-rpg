...
export default class StarterVillage {
  constructor() {
    ...
    this.monsterSpawns = [
      ...
      // 精英怪
      { monsterId: "eliteRabbitDemon", positions: [{ x: -25, y: 0, z: -12 }] },
      { monsterId: "eliteStoneGolem", positions: [{ x: 5, y: 0, z: -25 }] },
    ];
  }
  ...
  createMonsters() {
    this.monsterSpawns.forEach((spawn) => {
      spawn.positions.forEach((pos) => {
        ...
        // 如果是精英怪，添加脚底光圈到场景
        const eliteRing = monster.getEliteRing();
        if (eliteRing) {
