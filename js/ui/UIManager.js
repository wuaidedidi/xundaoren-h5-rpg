...
export default class UIManager {
  constructor(game) {
    ...
    // 商店状态
    this.shopMode = null; // 'buy', 'sell', 'repair'
  }
  ...
  updateCharacterPanel(player) {
    ...
    // 获取已装备物品
    const weapon = player.getEquippedItem("weapon");
    const armor = player.getEquippedItem("armor");
    const accessory = player.getEquippedItem("accessory");
    ...
    content.innerHTML = `
            <div class="character-stats">
                ...
                <div class="stat-row stat-highlight"><span>攻击力</span><span>${player.attack}</span></div>
                <div class="stat-row stat-highlight"><span>防御力</span><span>${player.defense}</span></div>
                <div class="stat-row"><span>移动速度</span><span>${player.speed}</span></div>
                <div class="stat-row"><span>暴击率</span><span>${(player.critRate * 100).toFixed(1)}%</span></div>
                <div class="stat-row"><span>伤害减免</span><span>${(player.defenseRate * 100).toFixed(1)}%</span></div>
                <div class="stat-row"><span>金币</span><span>${player.gold}</span></div>
            </div>
            <div class="equipment-section">
                <h4>装备栏</h4>
                <div class="equipment-slots">
                    <div class="equipment-slot" data-slot="weapon" onclick="game.handleEquipmentSlotClick('weapon')">
                        <div class="slot-label">⚔️ 武器</div>
                        ${
                          weapon
                            ? `
                            <div class="equipped-item" style="border-color: ${getRarityBorderColor(weapon.rarity)}">
                                <div class="item-icon">${weapon.icon}</div>
                                <div class="item-name" style="color: ${getRarityColor(weapon.rarity)}">${
                                weapon.name
                              }</div>
                                <div class="item-stats">${this.formatItemStats(weapon)}</div>
                            </div>
                        `
                            : '<div class="slot-empty">空</div>'
                        }
                    </div>
                    <div class="equipment-slot" data-slot="armor" onclick="game.handleEquipmentSlotClick('armor')">
                        <div class="slot-label">🛡️ 护甲</div>
                        ${
                          armor
                            ? `
                            <div class="equipped-item" style="border-color: ${getRarityBorderColor(armor.rarity)}">
                                <div class="item-icon">${armor.icon}</div>
                                <div class="item-name" style="color: ${getRarityColor(armor.rarity)}">${
                                armor.name
                              }</div>
                                <div class="item-stats">${this.formatItemStats(armor)}</div>
                            </div>
                        `
                            : '<div class="slot-empty">空</div>'
                        }
                    </div>
                    <div class="equipment-slot" data-slot="accessory" onclick="game.handleEquipmentSlotClick('accessory')">
                        <div class="slot-label">💍 饰品</div>
                        ${
                          accessory
                            ? `
                            <div class="equipped-item" style="border-color: ${getRarityBorderColor(accessory.rarity)}">
                                <div class="item-icon">${accessory.icon}</div>
                                <div class="item-name" style="color: ${getRarityColor(accessory.rarity)}">${
                                accessory.name
                              }</div>
                                <div class="item-stats">${this.formatItemStats(accessory)}</div>
                            </div>
                        `
                            : '<div class="slot-empty">空</div>'
                        }
  ...
  formatItemStats(item) {
    ...
    if (stats.hpBonus) parts.push(`生命+${stats.hpBonus}`);
    if (stats.mpBonus) parts.push(`法力+${stats.mpBonus}`);
    if (stats.critRate) parts.push(`暴击+${(stats.critRate * 100).toFixed(0)}%`);
    if (stats.defenseRate) parts.push(`减伤+${(stats.defenseRate * 100).toFixed(0)}%`);
    if (stats.hpRegen) parts.push(`回血+${stats.hpRegen}/s`);
  ...
  updateInventoryPanel(player) {
    if (this.elements.inventoryPanel) {
      ...
      if (grid) {
        // 清理之前创建的 tooltip 元素
        document.querySelectorAll(".item-tooltip").forEach((el) => el.remove());
        ...
        for (let i = 0; i < 24; i++) {
          ...
          if (invItem) {
            ...
            if (itemData) {
              ...
              const rarityColor = getRarityColor(displayItem.rarity || "common");
              const rarityBorder = getRarityBorderColor(displayItem.rarity || "common");
              const isEquipment = displayItem.slot;
              ...
              slot.innerHTML = `
                                <div class="item-icon" style="color: ${rarityColor}">${displayItem.icon}</div>
                                <div class="item-count">${displayItem.count > 1 ? displayItem.count : ""}</div>
                            `;

              // 创建 tooltip 元素并添加到 body
              const tooltip = document.createElement("div");
              tooltip.className = "item-tooltip";
              tooltip.id = tooltipId;
              tooltip.innerHTML = `
                                <div class="item-name" style="color: ${rarityColor}">${displayItem.name} ${
                isEquipment ? `<span class="rarity-tag">${getRarityName(displayItem.rarity)}</span>` : ""
              }</div>
                                <div class="item-desc">${displayItem.description}</div>
                                ${
                                  isEquipment
                                    ? `<div class="item-stats">${this.formatItemStats(displayItem)}</div>`
                                    : ""
                                }
                                ${
                                  isEquipment
                                    ? '<div class="item-hint">点击装备</div>'
                                    : '<div class="item-hint">点击使用</div>'
                                }
                            `;
   ...
   * 显示商店面板
   */
  showShopPanel(mode, player) {
    this.shopMode = mode;
    const panel = this.elements.shopPanel;
    if (!panel) return;

    const titleEl = panel.querySelector(".shop-title");
    const contentEl = panel.querySelector(".shop-content");
    const goldEl = panel.querySelector(".shop-gold");

    if (goldEl) goldEl.textContent = `持有金币: ${player.gold}`;

    contentEl.innerHTML = "";

    if (mode === "buy") {
      titleEl.textContent = "购买装备";
      ...
      shopItems.forEach((itemId) => {
        ...
        itemEl.style.borderColor = getRarityBorderColor(item.rarity);

        itemEl.innerHTML = `
                    <div class="shop-item-icon" style="color: ${getRarityColor(item.rarity)}">${item.icon}</div>
                    <div class="shop-item-info">
                        <div class="shop-item-name" style="color: ${getRarityColor(item.rarity)}">${item.name}</div>
        ...
        buyBtn.addEventListener("click", () => {
          if (this.game && this.game.handleBuyItem) {
            this.game.handleBuyItem(item.id);
          }
        });
      ...
      player.inventory.forEach((invItem, index) => {
        ...
        const sellPrice = Math.floor(item.price * 0.5); // 出售价格为购买价的一半

        const itemEl = document.createElement("div");
        itemEl.className = "shop-item";
        itemEl.style.borderColor = getRarityBorderColor(item.rarity || "common");

        itemEl.innerHTML = `
                    <div class="shop-item-icon" style="color: ${getRarityColor(item.rarity || "common")}">${
          item.icon
        }</div>
                    <div class="shop-item-info">
                        <div class="shop-item-name" style="color: ${getRarityColor(item.rarity || "common")}">${
          item.name
        }</div>
                        <div class="shop-item-desc">${item.description}</div>
                        <div class="shop-item-count">持有: ${invItem.count}</div>
                    </div>
                    <div class="shop-item-price">${sellPrice}💰</div>
                    <button class="shop-sell-btn" data-item-id="${item.id}" data-index="${index}">出售</button>
                `;

        const sellBtn = itemEl.querySelector(".shop-sell-btn");
        sellBtn.addEventListener("click", () => {
          if (this.game && this.game.handleSellItem) {
            this.game.handleSellItem(item.id, index);
          }
        });
      ...
      if (player.inventory.length === 0) {
        contentEl.innerHTML = '<div class="shop-empty">背包为空，没有可出售的物品</div>';
      }
    } else if (mode === "repair") {
      titleEl.textContent = "修理装备";
      ...
      slots.forEach((slot) => {
        ...
        itemEl.innerHTML = `
                    ...
                    <div class="shop-item-price">修理: ${repairCost}💰</div>
                    <button class="shop-repair-btn" data-slot="${slot}">修理</button>
                `;

        const repairBtn = itemEl.querySelector(".shop-repair-btn");
        repairBtn.addEventListener("click", () => {
          if (this.game && this.game.handleRepairItem) {
            this.game.handleRepairItem(slot);
          }
        });
      ...
      if (!hasEquipment) {
        contentEl.innerHTML = '<div class="shop-empty">没有装备需要修理</div>';
      }
    }
   ...
   * 更新商店金币显示
   */
  updateShopGold(player) {
    const goldEl = this.elements.shopPanel?.querySelector(".shop-gold");
    if (goldEl) {
      goldEl.textContent = `持有金币: ${player.gold}`;
    }
  }
