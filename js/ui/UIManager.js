...
import { getItem, getShopItems, RARITY } from "../data/items.js";
...
export default class UIManager {
  ...
  openShop(npc, mode = "buy") {
    ...
    panel.classList.remove("hidden");
    this.updateShopPanel(npc, mode);
  }

  /**
   * 更新商店面板
   */
  updateShopPanel(npc, mode) {
    const titleEl = document.getElementById("shop-title");
    const goldEl = document.getElementById("shop-player-gold");
    const itemsContainer = document.getElementById("shop-items");

    if (!titleEl || !goldEl || !itemsContainer) return;

    titleEl.textContent = `${npc.name} - ${mode === "buy" ? "购买装备" : mode === "sell" ? "出售物品" : "修理装备"}`;
    goldEl.textContent = this.game.player.gold;
    ...
    if (mode === "buy") {
      const shopItems = getShopItems(npc.shopItems || []);
      shopItems.forEach((item) => {
        const itemEl = this.createShopItemElement(item, mode);
      ...
      this.game.player.inventory.forEach((invItem) => {
        ...
        if (!item) return;
        const displayItem = { ...item, count: invItem.count };
        const itemEl = this.createShopItemElement(displayItem, mode);
        itemsContainer.appendChild(itemEl);
      });
    } else if (mode === "repair") {
      for (const slot in this.game.player.equipment) {
      ...
      this.game.player.inventory.forEach((invItem) => {
        if (!invItem) return;
        const item = getItem(invItem.itemId);
        if (item && item.type === "equipment" && item.durability !== undefined) {
          const itemEl = this.createShopItemElement(item, mode);
          itemsContainer.appendChild(itemEl);
        }
      });
   ...
   * 创建商店物品元素
   */
  createShopItemElement(item, mode) {
    const rarity = RARITY[item.rarity] || RARITY.common;
    const sellPrice = Math.floor((item.price || 0) * 0.5);
    const repairCost = Math.floor(((item.maxDurability || 100) - (item.durability || 100)) * 0.1);

    const el = document.createElement("div");
    el.className = "shop-item";
    el.dataset.itemId = item.id;
    el.innerHTML = `
            <div class="shop-item-icon" style="color: ${rarity.color}; border-color: ${rarity.borderColor}">${
      item.icon
    }</div>
            <div class="shop-item-info">
                ...
                ${
                  item.stats
                    ? `<div class="shop-item-stats">${Object.entries(item.stats)
                        .map(([k, v]) => `${k}: +${v}`)
                        .join(", ")}</div>`
                    : ""
                }
            ...
            <div class="shop-item-action">
                ${
                  mode === "buy"
                    ? `<button class="shop-buy-btn" data-price="${item.price}">${item.price} 灵石</button>`
                    : ""
                }
                ${
                  mode === "sell"
                    ? `<button class="shop-sell-btn" data-price="${sellPrice}">${sellPrice} 灵石</button>`
                    : ""
                }
                ${
                  mode === "repair"
                    ? `<button class="shop-repair-btn" data-cost="${repairCost}" ${repairCost <= 0 ? "disabled" : ""}>${
                        repairCost <= 0 ? "完好" : `修理 ${repairCost} 灵石`
                      }</button>`
                    : ""
                }
                ${item.count > 1 ? `<div class="shop-item-count">x${item.count}</div>` : ""}
            </div>
        `;
    return el;
  }
  ...
  bindShopEvents() {
    ...
    document.querySelectorAll(".shop-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        this.updateShopPanel(this.currentShopNPC, mode);
      });
    });
    ...
    document.querySelectorAll(".shop-buy-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (this.game.player.addItem(item)) {
          ...
          this.updateShopPanel(this.currentShopNPC, "buy");
        } else {
          this.showToast("背包空间不足", "warning");
        }
      });
    });
    ...
    document.querySelectorAll(".shop-sell-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (this.game.player.removeItem(itemId)) {
          this.game.player.gold += price;
          this.showToast(`出售了${item.name}`, "success");
          this.updateShopPanel(this.currentShopNPC, "sell");
        }
      });
    });

    document.querySelectorAll(".shop-repair-btn:not(:disabled)").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        for (const slot in this.game.player.equipment) {
          if (this.game.player.equipment[slot] === itemId) {
            const item = getItem(itemId);
            if (item) {
              item.durability = item.maxDurability || 100;
              this.game.player.gold -= cost;
              this.showToast(`修理了${item.name}`, "success");
              this.updateShopPanel(this.currentShopNPC, "repair");
              return;
            }
          }
        }

        const invItem = this.game.player.inventory.find((i) => i && i.itemId === itemId);
        if (invItem) {
          const item = getItem(itemId);
          if (item) {
            item.durability = item.maxDurability || 100;
  ...
  updateEquipmentPanel(player) {
    for (const slot in player.equipment) {
      ...
      if (itemId) {
        ...
        if (item) {
          const rarity = RARITY[item.rarity] || RARITY.common;
          iconEl.innerHTML = `<span style="color: ${rarity.color}">${item.icon}</span>`;
          slotEl.style.borderColor = rarity.borderColor;
  ...
  bindEquipmentEvents() {
    document.addEventListener("click", (e) => {
      ...
      if (result.success) {
        ...
        this.updateInventoryPanel(this.game.player);
        this.updateEquipmentPanel(this.game.player);
      }
    });
  }
}
